package PSRVTest
  model ms
    ...
    extends Simulator.Files.ThermodynamicPackages.PengRobinson;
      ...
      model Test
              ...
              parameter data.Methane ace;
              parameter data.Ethane eth;
              parameter data.Ethylene wat;
              parameter data.GeneralProperties C[Nc] = {ace, eth, wat};
              PSRVTest.ms S1(C = C, Nc = Nc) annotation(
  ...
  equation
    ...
    m_c = p_c .- 0.0089 .* (1.0 .+ Tr_c .^ 0.5) .* (0.7 .- Tr_c);
    q_c = 0.45724 * R ^ 2 * C.Tc .^ 2 ./ C.Pc;
    a_c = q_c .* (1 .+ m_c .* (1 .- sqrt(Tr_c))) .^ 2;
    p_c = 0.378893 .+ 1.4897153 * C.AF .- 0.17131848 * C.AF .^ 2 .+ 0.0196554 * C.AF .^ 3;
